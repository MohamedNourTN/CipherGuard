"""
Copyright © 2024 Mohamed Nour

"""

__title__ = "CipherGuard"
__author__ = "Mohamed Nour"
__license__ = "MIT"
__copyright__ = "Copyright © 2024 Mohamed Nour"
__version__ = "1.0.0"
...
def exception_logged(function: Callable):
    ...
    and log the traceback to CipherGuard.log file in that case.
    """
    @functools.wraps(function)
    def wrapper(*args, **kwargs):
        try:
        ...
        if self.master.__class__.__name__ == Interface.__name__:
            ...
            if self.index(self.select()) == 4:
                ...
                if not hasattr(self, "HTML"):
                    ...
                    self.master.update()
                    try:
                        request = get("https://raw.githubusercontent.com/MohamedNourTN/CipherGuard/main/README.md").text
                ...
                if self.index(self.select()) == 5 and hasattr(self.master, "_sourceLoadFailure") and self.master._sourceLoadFailure:
                    ...
                    try:
                        ...
                        url = [release["zipball_url"] for release in get(f"https://api.github.com/repos/MohamedNourTN/{__title__}/releases").json() if release["tag_name"] == f"v{__version__}"][0]
...
class Interface(Tk):
    ...
    def __init__(self):
        ...
        class mainNotebook(Notebook):
            def __init__(self, master: Interface):
                    ...
                    def showDebug(self, event=None):
                        class debugWindow(Toplevel):
                            def __init__(self, master: encryptionFrame, root: Interface):
                                ...
                                self.wm_title("CipherGuard File Selection Debugging")
                ...
                self.add(self.miscFrame, text="Hashing")
    ...
    def __save_database(self):
        con = sqlite3.connect(f"{os.getenv('APPDATA')}\\CipherGuard\\settings.sqlite")
        cur = con.cursor()
    ...
    def __load_database(self):
        ...
        try:
            ...
            os.mkdir(f"{os.getenv('APPDATA')}\\CipherGuard")
            con = sqlite3.connect(f"{os.getenv('APPDATA')}\\CipherGuard\\settings.sqlite")
            ...
            def __init__(self, master: Interface):
                ...
                class fileMenu(Menu):
                    def __init__(self, master: menuBar):
                        ...
                        self.add_command(label = "Hashing", command=lambda: self.master.master.mainNotebook.select(2), accelerator="Ctrl+M", underline=0)
                        self.add_separator()
                        self.add_separator()
                ...
                self.add_cascade(label = "CipherGuard | Mohamed Nour", menu=self.fileMenu)
    ...
    class Settings(Toplevel):
        def __init__(self, master: Tk):
            ...
            self.wm_title("CipherGuard Settings")
            self.wm_geometry(f"{self.width}x{self.height}")
            self.wm_resizable(height=False, width=False)
            self.wm_attributes("-fullscreen", False)
            self.wm_maxsize(self.width, self.height)
